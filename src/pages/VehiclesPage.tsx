...
import { SWAPI_Vehicles, SWAPI_Search_Vehicles } from '../types'
...
const VehiclesPage = () => {
	...
	const [data, setData] = useState<SWAPI_Vehicles[] | null>(null)
	const [searchParams, setSearchParams] = useSearchParams()
	const [searchResult, setSearchResult] = useState<SWAPI_Search_Vehicles | null>(null)
	...
	const changePage = async (next: boolean) => {
		...
		try {
			const result = await getNewPageData<SWAPI_Search_Vehicles>(searchResult.next_page_url)
			setData(result.data)
	...
	const getData = async (query: string, page: number) => {
		...
		try {
			const result = await searchResource<SWAPI_Search_Vehicles>("vehicles", query, page)
			setData(result.data)
			setSearchResult(result)
			setPage(page)
